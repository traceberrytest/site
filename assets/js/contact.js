...
$(function() {
  ...
  const contactButton = document.querySelector('button[name="contactButton"]');
  const contactResponse = document.querySelector('h5[name="contactResponse"]');
  ...
  contactUsForm.onsubmit = e => {
    ...
    let data = {};
    Array.from(contactUsForm).map(input => (data[input.id] = input.value));
    data['type'] = 'contact';
    console.log('Sending: ', JSON.stringify(data));
    contactButton.disabled = true;
    contactResponse.innerHTML = 'Sending...'

    // Create the AJAX request
    var xhr = new XMLHttpRequest();
    xhr.open(contactUsForm.method, formURL, true);
    ...
    xhr.onloadend = response => {
      if (response.target.status === 200) {
        contactUsForm.reset();
        contactResponse.innerHTML = 'Request submitted.';
      } else {
        contactResponse.innerHTML = 'Please try again.';
